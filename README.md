...
[![GitHub stars](https://img.shields.io/github/stars/manhpd98/multi-agent-coding?style=social)](https://github.com/manhpd98/multi-agent-coding)
...
Multi-Agent Coding automatically:
...
ai-pilot task claude "Fix null pointer in auth.ts"   # Protocol-based task (YAML → worker → result JSON)
...
| `ai-pilot task <worker> "<description>"` | Protocol-based delegation: generates a task YAML, sends to worker, reads result JSON. Minimal token usage via `CLAUDE.md` protocol. |
...
## Protocol-Based Delegation (v2.0)

The `task` command uses a structured protocol for minimal token usage:

1. **Pilot generates** a task YAML file (`.ai-pilot/tasks/task-<ID>.yaml`)
2. **Worker reads** the YAML + `CLAUDE.md` instructions (no large SKILL.md needed)
3. **Worker writes** a result JSON (`.ai-pilot/results/task-<ID>.json`)
4. **Pilot reads** the result and decides next steps

This approach saves tokens by replacing verbose prompt templates with a structured file-based protocol. Workers like Claude Code automatically read `CLAUDE.md` for instructions, so no prompt engineering is needed per task.
...
.ai-pilot/
├── tasks/                                 # Task YAML files (generated by Pilot)
├── results/                               # Result JSON files (written by Worker)
├── logs/                                  # Delegation session logs
├── hooks/
│   └── on-complete.sh                     # Auto-run checks after task completion
└── schema/
    ├── task.yaml                          # Task YAML schema reference
    └── result.json                        # Result JSON schema reference

CLAUDE.md                                  # Protocol instructions for Claude Code worker
...
| **Token Usage** | Verbose prompts every time | Protocol-based: YAML in, JSON out |
...
Multi-Agent Coding is free (MIT license). Workers may have costs: Claude Code needs Claude Pro ($20/mo), OpenCode includes free models, Aider uses your own API keys.
</details>
...
Yes. Multi-Agent Coding is language-agnostic. It includes platform configs for iOS, Android, Web, Flutter, and Python, but works with any tech stack.
</details>

<details>
<summary><b>How is this different from just using Claude Code directly?</b></summary>
When you use Claude Code alone, it plans AND codes in the same context. Multi-Agent Coding separates these roles so the Pilot can focus on planning and review while the Coder focuses on implementation. The result is better code quality and fewer missed bugs.
</details>
